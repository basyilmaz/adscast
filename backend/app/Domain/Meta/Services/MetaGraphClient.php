    ...
    public function deleteObject(
        ...
        try {
            $response = $this->http
                ...
                ->retry(2, 500)
                ->timeout(30)
                ->delete("{$baseUrl}/{$apiVersion}/{$normalizedPath}", array_merge($data, [
                    'access_token' => $accessToken,
                ]));
        } catch (ConnectionException $exception) {
            throw new MetaApiException('Meta API DELETE baglanti hatasi.', [
                'path' => $normalizedPath,
                'api_version' => $apiVersion,
            ], previous: $exception);
        }
        ...
        if (! is_array($body)) {
            throw new MetaApiException('Meta API DELETE gecersiz response dondu.', [
                'path' => $normalizedPath,
                'api_version' => $apiVersion,
            ]);
        }
