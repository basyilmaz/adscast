...
class ReportDeliveryFoundationTest extends TestCase
{
    ...
    public function test_reports_index_includes_failure_resolution_action_analytics(): void
    {
        ...
        $accountTemplate = ReportTemplate::query()->create([
            'workspace_id' => $workspace->id,
            'name' => 'Action Analytics Account Template',
            'entity_type' => 'account',
            'entity_id' => $account->id,
            'report_type' => 'client_account_summary_v1',
            'default_range_days' => 14,
        ...
        $campaignTemplate = ReportTemplate::query()->create([
            'workspace_id' => $workspace->id,
            'name' => 'Action Analytics Campaign Template',
        ...
        $this->withHeader('Authorization', "Bearer {$token}")
            ...
            ->assertOk()
            ->assertJsonPath('data.action_code', 'review_contact_book');
        ...
        $indexResponse->assertOk()
            ->assertJsonPath('data.failure_resolution_action_analytics_summary.observed_actions', 2)
            ->assertJsonPath('data.failure_resolution_action_analytics_summary.tracked_interactions', 2)
            ->assertJsonPath('data.failure_resolution_action_analytics_summary.api_attempts', 1)
            ->assertJsonPath('data.failure_resolution_action_analytics_summary.route_interactions', 1)
            ->assertJsonPath('data.failure_resolution_action_analytics_summary.focus_interactions', 0)
            ->assertJsonPath('data.failure_resolution_action_analytics_summary.successful_executions', 1)
            ->assertJsonPath('data.failure_resolution_action_analytics_summary.partial_executions', 0)
            ->assertJsonPath('data.failure_resolution_action_analytics_summary.failed_executions', 0)
            ->assertJsonPath('data.failure_resolution_action_analytics_summary.best_success_action_label', 'Basarisiz teslimleri tekrar dene');

        $analytics = collect($indexResponse->json('data.failure_resolution_action_analytics'))->keyBy('action_code');

        $this->assertSame(2, $analytics->count());
        $this->assertSame(1, data_get($analytics->get('retry_failed_runs'), 'observed_uses'));
        $this->assertSame(1, data_get($analytics->get('retry_failed_runs'), 'api_attempts'));
        $this->assertSame(1, data_get($analytics->get('retry_failed_runs'), 'successful_executions'));
        $this->assertSame(0, data_get($analytics->get('retry_failed_runs'), 'failed_executions'));
        $this->assertSame('smtp_timeout', data_get($analytics->get('retry_failed_runs'), 'top_reason_code'));
        $this->assertSame(1, data_get($analytics->get('review_contact_book'), 'observed_uses'));
        $this->assertSame(1, data_get($analytics->get('review_contact_book'), 'route_interactions'));
        $this->assertNull(data_get($analytics->get('review_contact_book'), 'success_rate'));
        $this->assertSame('recipient_rejected', data_get($analytics->get('review_contact_book'), 'top_reason_code'));
    ...
    public function test_reports_index_includes_featured_failure_resolution_analytics(): void
    {
        [$workspace, $token, $account, $campaign] = $this->seedReportFixture('agency.admin@adscast.test');

        $accountTemplate = ReportTemplate::query()->create([
            'workspace_id' => $workspace->id,
            'name' => 'Featured Analytics Account Template',
            'entity_type' => 'account',
            'entity_id' => $account->id,
            'report_type' => 'client_account_summary_v1',
            'default_range_days' => 14,
        ...
        $campaignTemplate = ReportTemplate::query()->create([
            'workspace_id' => $workspace->id,
            'name' => 'Featured Analytics Campaign Template',
            'entity_type' => 'campaign',
            'entity_id' => $campaign->id,
            'report_type' => 'client_campaign_summary_v1',
            'default_range_days' => 7,
        ...
        $this->withHeader('Authorization', "Bearer {$token}")
            ->withHeader('X-Workspace-Id', $workspace->id)
            ->postJson("/api/v1/reports/failure-resolution-actions/campaign/{$campaign->id}/review_recipient_groups/track")
            ->assertOk()
        ...
        $indexResponse->assertOk()
            ->assertJsonPath('data.featured_failure_resolution_analytics_summary.tracked_interactions', 2)
            ->assertJsonPath('data.featured_failure_resolution_analytics_summary.featured_interactions', 1)
            ->assertJsonPath('data.featured_failure_resolution_analytics_summary.override_interactions', 1)
            ->assertJsonPath('data.featured_failure_resolution_analytics_summary.featured_api_attempts', 1)
        ...
        $this->assertSame(1, data_get($analytics->get('smtp_timeout'), 'featured_interactions'));
        $this->assertSame(0, data_get($analytics->get('smtp_timeout'), 'override_interactions'));
        $this->assertEquals(100.0, data_get($analytics->get('smtp_timeout'), 'featured_success_rate'));
        $this->assertSame('review_contact_book', data_get($analytics->get('recipient_rejected'), 'featured_action_code'));
