...
class CampaignDrillDownTest extends TestCase
{
    ...
    public function test_campaign_detail_returns_tabs_ready_drill_down_payload(): void
    {
        ...
        AuditLog::query()->create([
            ...
            'target_id' => null,
            'metadata' => [
                'recommendation_code' => 'review_data_validation',
                'recommendation_label' => 'Veri bloklarini gozden gecir',
                ...
                'guidance_message' => 'Veri dogrulamasi bekleyen bloklar icin once manuel gozden gecirme gerekli.',
                'target_count' => 2,
                'attempted_count' => 2,
                'successful_count' => 1,
                'failed_count' => 1,
                'outcome_status' => 'partial',
                'reason_codes' => ['waiting_data_validation'],
                'priority_group_keys' => ['waiting_data_validation'],
                'target_entity_types' => ['campaign'],
        ...
        $response->assertOk()
            ...
            ->assertJsonPath('data.recipient_group_failure_alignment_summary.tracked_failed_runs', 0)
            ->assertJsonPath('data.recipient_group_failure_alignment_summary.overridden_failed_runs', 0)
            ->assertJsonCount(0, 'data.recipient_group_failure_alignment')
            ...
            ->assertJsonPath('data.decision_queue_recommendation_analytics.0.recommendation_code', 'review_data_validation')
            ->assertJsonPath('data.decision_queue_recommendation_analytics.0.application_success_rate', 0)
            ->assertJsonPath('data.decision_queue_recommendation_analytics.0.item_success_rate', 50)
            ->assertJsonPath('data.decision_queue_recommendation_analytics.0.entities.0.entity_type', 'campaign')
            ...
            ->assertJsonCount(0, 'data.retry_recommendations')
            ->assertJsonPath('data.failure_resolution_summary.total_actions', 0)
            ->assertJsonPath('data.failure_resolution_summary.retryable_runs', 0)
            ->assertJsonCount(0, 'data.failure_resolution_actions')
    ...
    public function test_campaign_detail_surfaces_recipient_specific_resolution_actions_for_rejected_recipients(): void
    {
        ...
        $response->assertOk()
            ...
            ->assertJsonPath('data.featured_failure_resolution.status', 'blocked_retry')
            ->assertJsonPath('data.featured_failure_resolution.action_code', 'review_contact_book')
            ->assertJsonPath('data.featured_failure_resolution.reason_code', 'recipient_rejected')
