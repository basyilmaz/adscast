...
export default function DashboardPage() {
  ...
  const {
    ...
    isRefreshing,
    reload,
  } = useApiQuery<DashboardOverviewResponse, DashboardOverviewResponse["data"]>(
    buildApiPathWithFilters("/dashboard/overview", searchParams, GLOBAL_DATE_FILTER_KEYS),
    {
      requestOptions: {
  ...
  return (
    <div className="space-y-4">
      ...
      {error ? <PageErrorState title="Dashboard verisi alinamadi" detail={error} /> : null}
                  ...
                  <div>
                    <Link
                      href={buildHrefWithFilters(
                        `/ad-accounts/detail?id=${encodeURIComponent(account.id)}`,
                        searchParams,
